...
INITIALIZE_ENTITY_STORAGE(mingl_first)
...
int init(GLFWwindow *window)
{
    ...
    mingl_first_create_entity_tables();
    ...
    for (int x = 0; x < files.len; x++)
    {
        ...
        model *current_model;
        mingl_first_model_factory(&current_model);
        load_model(filename, current_model, &mingl_first_mesh_factory);

        // add the new model to a simple scene
        scene *current_scene;
        mingl_first_scene_factory(&current_scene);
    ...
    scene *lamp;
    mingl_first_scene_factory(&lamp);
    lamp_scene(lamp, &positional_light, lamp_material);

    // register a box into the world
    scene *box;
    mingl_first_scene_factory(&box);
    lamp_scene(box, NULL, container_material);
    box->update = &container_update;

    // initialize all scenes
    for (size_t i = 0; i < mingl_first_scenes.len; i++)
    {
        scene *current_scene = get_slice_item(&mingl_first_scenes, i);
        current_scene->init(current_scene, &mingl_first_mesh_factory, &mingl_first_model_factory);
    }

    // send all models to opengl
    for (size_t i = 0; i < mingl_first_models.len; i++)
    {
        model *current_model = get_slice_item(&mingl_first_models, i);
...
void update(GLFWwindow *window, double time, double dt)
{
    ...
    scene *scene_data = (scene *)mingl_first_scenes.data;
    for (int x = 0; x < mingl_first_scenes.len; x++)
    {
